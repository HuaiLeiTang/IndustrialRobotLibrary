...
MultiANN::MultiANN(					// constructor
				   ...
				   int *x_ptr,					// pointer to the object holding the initial point
				   ...
				   ANNpoint ss)					// scaling of the coordinates
{
	...
	points_ptr = new int[MAXPOINTS]; 	       	// allocate pointers to the data points
...
MultiANN::MultiANN(						// constructor
				   int dim, int Maxpoint, 			// dimension of the space
				   ANNpoint x_coor, 	// coordinates of the initial point in the data structure
				   int *x_ptr)			// pointer to the object holding the initial point
				   						...
				   						// scaling of the coordinates
{
	...
	topology = new int[dimension]; 			// allocate topology array
	scaling = annAllocPt(dimension); 			// allocate scaling array
	points_coor = annAllocPts(Maxpoint, dimension);     	// allocate coordinates of the data points
	points_ptr = new int[Maxpoint]; 	       	// allocate pointers to the data points
									...
									// scaling of the coordinates
{
	...
	topology = new int[dimension];			// allocate topology array	
	scaling = annAllocPt(dimension); 			// allocate scaling array
	points_coor = annAllocPts(Maxpoint, dimension); 	// allocate data points
	points_ptr = new int[Maxpoint]; 			// allocate pointers to the data points
...
void MultiANN::AddPoint(				// dynamic update of the data points
						...
						int *x_ptr)			// pointer to the point
{
	for (int i = 0; i < dimension; i++) {
							  ...
							  double* best_dist) 		// distance from the nearest neighbor to x (returned)
{
	...
	*best_dist = d_min;
	//return points_ptr[(int)best_idx];
}
...
void MultiANN::NearestNeighbor(const ANNpoint &x,      	// query point
							   ...
							   int *&best_ptr)       	// pointers to the nearest neighbors to x (returned)
{
