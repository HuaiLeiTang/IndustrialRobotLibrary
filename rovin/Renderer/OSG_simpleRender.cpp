...
namespace rovin
{
	...
	OSG_simpleRender::OSG_simpleRender(const SerialOpenChain& socRobot, const State& state, int width, int height)
	{
		...
		for (unsigned int i = 0; i < dof + 1; i++)
		{
			...
			transformNode->setMatrix(convertMatrix(state.getLinkStateSE3(i)));
