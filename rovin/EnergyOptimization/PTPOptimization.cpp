...
namespace rovin{
	...
	void PTPOptimization::makeBSplineKnot()
	{
		...
		//}
		_knot.resize(_orderOfBSpline + _numOfOptCP + 6 + 1);
		...
		Real delta = _tf / (_numOfOptCP + 6 - _orderOfBSpline);
		for (unsigned int i = 0; i < _numOfOptCP + 6 - _orderOfBSpline; i++)
		{
	...
	sharedResource::sharedResource(PTPOptimization* PTPOptimizer)
	{
		...
		MatrixX cp(1, _PTPOptimizer->_numOfOptCP + 6); /// control points including boundary control points
